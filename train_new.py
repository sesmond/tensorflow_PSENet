    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options, allow_soft_placement=True)) as sess:
        ...
        for step in range(FLAGS.max_steps):
            ...
            if step % FLAGS.save_checkpoint_steps == 0:
                ...
                save_model(global_step,sess,tl,input_images,seg_maps_pred)
...
def save_model(global_step, sess, tl,input_images,seg_maps_pred):
    # 模型名为了不冲突最好加上别的名字
    ...
    model_util.save_model(sess,global_step,FLAGS.checkpoint_path,input_images,seg_maps_pred)
